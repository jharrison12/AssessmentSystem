...
def student_page(request, edclass, semester, assignmentname):
    ...
    if request.method == 'POST':
        ...
        students = Student.objects.filter(enrollment__rubricdata__rubriccompleted=False, edclasses=edclassesPulled,
                                          enrollment__rubricdata__assignment=assignment).distinct()
...
def rubric_page(request, edclass, studentname, semester, assignmentname):
    ...
    greatEnrollment = Enrollment.objects.get(student=student,
                                             edclass=edclassenrolled)  # , rubricdata__rubricdata__rubriccompleted=False)
    ...
    if request.method == "POST":
        ...
        classassignment = Assignment.objects.get(
            pk=assignmentpk)  # edclass=edclassenrolled,  assignmentname=assignmentname)
        rubricforclass = classassignment.keyrubric
        ...
        rubricforclass.name = "{}{}{}{}".format(edclass, studentname, semester,
                                                re.sub(' ', '', classassignment.assignmentname))
        ...
        try:
            ...
            for row in rows:
                standardsall = row.standards.all()
                logging.warning("Standards is {} Row is {}".format(standardsall, row))
                row.pk = None
                row.rubric = rubricforclass
                logging.warning("THE RUBRIC FOR CLASS IS: %d" % rubricforclass.id)
                row.save()
                logging.critical("ROW SET? {}".format(row.standards))
                #row.standards.set(standardsall)
                [row.standards.add(standard) for standard in standardsall]
            ...
            if rubricdata[0].rubriccompleted == False:
                ...
                rubricname = "{}{}{}{}".format(edclass, studentname, semester,
                                               re.sub(' ', '', classassignment.assignmentname))
